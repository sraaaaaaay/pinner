...
2. Compare the user prompt(s) against those keywords and search within `./pins` for the relevant file. For example, if the user is asking about authentication, Claude should query that index for `auth`, `jwt` and so on.
